...
public class StringUtil {
    ...
    public static String getActionName(String menuPath) {
        ...
        if (null == arr) {
            ...
            if (arr.length > 2) {
                name = arr[arr.length - 2].toString();
            }
        }
        return name;
    ...
    /**
	 * 截取字符串长度，以所占的字节数判断，更好的处理中午和英文的区别.
	 * @param text
	 * @param length
	 * @return
	 */
	public static String subStringByByte(String text, int length) {
        int textLength = text.length();
        int byteLength = 0;
        StringBuffer returnStr = new StringBuffer();
        for (int i = 0; i < textLength && byteLength < length * 2; i++) {
            String subSstr = text.substring(i, i + 1);
            if (subSstr.getBytes().length == 1) {
                //英文
                byteLength++;
            } else {//中文
                byteLength += 2;
            }
            returnStr.append(subSstr);
        }
        try {
